...
def login(user, request):
    access_token = user.create_access_token(request=request, fresh=timedelta(minutes=15))
...
def logout_user(user, response=None):
    method = f'logout_user({self}, response={response}, logout_all_sessions={logout_all_sessions})'
    ...
    if response:
        if not logout_all_sessions:  # if we logged out all sessions, then all tokens have already been removed
            ...
            if not access_cookie_value:
                _logger.warning(
                    f'{method}: no access_cookie_value for user #{self.id}, cannot invalidate access token')
            else:
                found_tokens = models.AccessToken.query.filter_by(user_id=self.id, token=access_cookie_value).all()
                if not found_tokens:
            ...
            if not refresh_cookie_value:
                ...
                found_tokens = models.RefreshToken.query.filter_by(
                    user_id=self.id, token=refresh_cookie_value).all()
                if not found_tokens:
                    _logger.warning(
    ...
    def remove_expired_tokens(self):
        ...
        user_tokens = models.AccessToken.query.filter_by(user_id=self.id).all() + \
                      models.RefreshToken.query.filter_by(user_id=self.id).all()
    ...
    def create_access_token(self, request=None, fresh=False, expires_delta=timedelta(minutes=15)):
        ...
        access_token = jwt.create_access_token(identity=self.id, fresh=fresh) # set JWT access cookie (includes CSRF)
